    ...
    },
    {
      ...
      body: row => {
        return (
          <input
            type="checkbox"
            checked={produtoSelecionadoEstProdOrigem?.some(item => String(item.IDPRODUTO) === String(row.IDPRODUTO))}
            // checked={produtoSelecionadoEstProdOrigem?.includes(row.IDPRODUTO)}
            onChange={() =>
